...
if(filterForm){
 ...
 function readURL(){const q=new URLSearchParams(location.search);category=normalizeCategory(q.get('categoria')||location.hash.slice(1));sizeSelect.value=q.get('talla')||'';if(sizeSelect.selectedIndex<0)sizeSelect.value='';sortSelect.value=q.get('orden')||'original';if(sortSelect.selectedIndex<0)sortSelect.value='original';applyFilters(false)}
